  ...
  value: yup
    ...
    .test("number_check", function (nInput) {
      ...
      try {
        ...
        if (nInput <= 0)
          return this.createError({
            message: "Valor não pode ser igual ou menor a zero.",
            path: this.path,
          });
      } catch (e) {
        return this.createError({
          message: "Valor inserido é inválido.",
          path: this.path,
        });
      }
...
export const forcaExplosivaSchema = yup.object().shape({
  impulsaoVertical: yup.array().of(numberValidation),
});
...
export const forcaIsometricaSchema = yup.object().shape({
  forcaIsometricaMaos: yup.array().of(minSecValidation),
});
export type forcaIsometricaSchemaType =
  typeof forcaIsometricaSchema.__outputType;

export const abdominalSchema = yup.object().shape({
  abdominais: yup.array().of(numberValidation),
});
export type AbdominalSchemaType = typeof abdominalSchema.__outputType;

export const mmssSchema = yup.object().shape({
  flexoes: yup.array().of(numberValidation),
});
export type MmssSchemaType = typeof mmssSchema.__outputType;

export const resistenciaAnaerobicaSchema = yup.object().shape({
  burpees: yup.array().of(numberValidation),
});
export type resistenciaAnaerobicaSchemaType =
  typeof resistenciaAnaerobicaSchema.__outputType;

export const resistenciaAaerobicaSchema = yup.object().shape({
  cooper: yup.array().of(numberValidation),
});
export type resistenciaAaerobicaSchemaType =
  typeof resistenciaAaerobicaSchema.__outputType;

export const mobilidadeTornozeloSchema = yup.object().shape({
  testeDeLunge: yup.array().of(
    yup.object().shape(
      {
        atletaId: yup.number(),
        atletaNome: yup.string(),
        esquerdo: yup
          ...
          .max(12, "Valor máximo permitido: 12")
          .nullable()
          .when("direito", ([direito], schema) => {
            if (direito) {
              return schema.required(
                "Não é permitido inserir o valor de somente um joelho."
              );
            }
            return schema;
          }),
...
export type mobilidadeTornozeloSchemaType =
  typeof mobilidadeTornozeloSchema.__outputType;
      ...
      peso: yup
        ...
        .test("weight_check", function (weightInput) {
          ...
          try {
            ...
            if (weightInput <= 0)
              return this.createError({
                message: "Peso não pode ser igual ou menor a zero.",
                path: this.path,
              });
          } catch (e) {
            return this.createError({
              message: "Peso inserido é inválido.",
              path: this.path,
            });
          }
        ...
        .test("height_check", function (heightInput) {
          ...
          try {
            if (isNaN(heightInput))
              throw new Error("Altura inserida é inválido");

            if (heightInput <= 0)
              return this.createError({
                message: "Altura não pode ser igual ou menor a zero.",
                path: this.path,
              });
          } catch (e) {
            return this.createError({
              message: "Altura inserida é inválido.",
              path: this.path,
            });
          }
