...
export default function ListAthletes({
  ...
  return (
    <FormWrapper header="Lista de Atletas" handleSubmit={handleSubmit}>
        ...
        {listAthletes?.data?.map((athlete) => (
          ...
          >
            <input
              ...
              name="ids"
              value={athlete?.id}
              className="xl:w-6 md:w-5 sm:w-5 w-5 xl:h-6 md:h-5 sm:h-5 h-5 bg-gray-100 rounded-md border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
            ></input>
            <span className="uppercase xl:text-lg md:text-base sm:text-base text-base whitespace-nowrap">
              {athlete?.nome}
            </span>
          </li>
        ))}
      ...
      {error && (
        <p className="xl:text-base md:text-sm sm:text-xs text-xs text-center text-orange-700 xl:mt-10 md:mt-7 sm:mt-5 mt-5 font-bold">
          {error}
        </p>
      )}
    </FormWrapper>
