...
const PosturaCard = ({atletaId, datasAvaliacaoArray, viewSide}: PosturaCardProps) => {
  ...
  const [isDisabled, setIsDisabled] = useState<boolean>(false);
    const [selectedOption, setSelectedOption] = useState<string|null>(datasAvaliacaoArray[0]);
    ...
    const handleOptionSelect = async (event: React.ChangeEvent<HTMLSelectElement>) => {
        const selectedOption = event.currentTarget.value != "info" ? event.currentTarget.value : null;
      ...
      const RenderDatasAvaliacaoOptions = ({ datasAvaliacaoArray }: { datasAvaliacaoArray: string[] }) => (
        <>
          { (datasAvaliacaoArray.length > 0) && <select id='possibleDates' className='p-2 mt-4 shadow mx-auto w-fit rounded' onChange={handleOptionSelect} value={selectedOption ? selectedOption : undefined}>
            <option key={"info"} disabled={isDisabled} value={"info"}>{dropdownHeader}</option>
